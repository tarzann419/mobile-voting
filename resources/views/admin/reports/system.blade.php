    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Completed Elections</p>
                                <p class="text-2xl font-bold text-gray-900 dark:text-gray-100">{{ $stats['completed_elections'] ?? 0 }}</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <div class="flex items-center">
                            ...
                            <div class="ml-4">
                                <p class="text-sm font-medium text-gray-500 dark:text-gray-400">Upcoming Elections</p>
                                <p class="text-2xl font-bold text-gray-900 dark:text-gray-100">
                                    {{ ($stats['total_elections'] ?? 0) - ($stats['active_elections'] ?? 0) - ($stats['completed_elections'] ?? 0) }}
                                </p>
                            </div>
                        </div>
                    </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">Monthly Organization Registrations</h3>
                        @if(isset($monthlyOrgs) && $monthlyOrgs->count() > 0)
                            <div class="space-y-3">
                                @foreach($monthlyOrgs as $month)
                                    <div class="flex justify-between items-center">
                                        ...
                                        <div class="flex items-center">
                                            <div class="w-32 bg-gray-200 dark:bg-gray-700 rounded-full h-2 mr-3">
                                                @php
                                                    $maxCount = $monthlyOrgs->max('count');
                                                    $percentage = $maxCount > 0 ? ($month->count / $maxCount) * 100 : 0;
                                                @endphp
                                                <div class="bg-blue-500 h-2 rounded-full" style="width: {{ $percentage }}%"></div>
                                            </div>
                                            <span class="text-sm font-medium text-gray-900 dark:text-gray-100">{{ $month->count }}</span>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
                        @else
                            <p class="text-gray-500 dark:text-gray-400">No organization registration data available.</p>
                        @endif
                    </div>
                </div>
                ...
                <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6">
                        <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">Monthly User Registrations</h3>
                        @if(isset($monthlyUsers) && $monthlyUsers->count() > 0)
                            <div class="space-y-3">
                        ...
                        @else
                            <p class="text-gray-500 dark:text-gray-400">No user registration data available.</p>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6">
                    <h3 class="text-lg font-medium text-gray-900 dark:text-gray-100 mb-4">Export Reports</h3>
